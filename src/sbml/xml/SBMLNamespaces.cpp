...
{
  std::list<SBMLNamespaces*>* result = new std::list<SBMLNamespaces*>();
  ...
  result->push_back(new SBMLNamespaces(2,5));
...
SBMLNamespaces::freeSBMLNamespaces(std::list<SBMLNamespaces*> * supportedNS)
{
  ...
  while (supportedNS->size())
  {
    delete supportedNS->back();
    supportedNS->pop_back();
  }
  delete supportedNS;
}
...
{
  int success = LIBSBXML_OPERATION_SUCCESS;
                                ...
                                const std::string &pkgPrefix)
{
  ...
  if (sbmlext)
  {
    ...
    {
      return LIBSBXML_INVALID_ATTRIBUTE_VALUE;
    }
  }
  else
  {
...
SBMLNamespaces::addNamespace(const std::string &uri, const std::string &prefix)
{
  ...
  return mNamespaces != NULL ? mNamespaces->add(uri, prefix) : LIBSBXML_INVALID_OBJECT;
}
...
SBMLNamespaces_getSupportedNamespaces(int *length)
{
  if (length == NULL) return NULL;
   std::list<SBMLNamespaces*>* supported = SBMLNamespaces::getSupportedNamespaces();
